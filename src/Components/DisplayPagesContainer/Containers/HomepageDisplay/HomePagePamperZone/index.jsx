...
const HomePagePamperZone = () => {
  ...
  useEffect(() => {
    if (banners == [] || !banners || banners?.length === 0) {
      setIsLoading(true);
      dispatch(getHomePagePamperZone()).then((res) => {
        setIsLoading(false);
      }).catch((err) => {
        setIsLoading(false);
      });
          ...
          <tr>
            <td>
              <div className="d-flex justify-content-center pt-4">
                <p className="text-red">Homepage Pamper Zone list is empty !!</p>
              </div>
            </td>
          </tr>
        )}
      ...
      <>
        <Col md={4}>
          <div className="user_heading">
            <h3>Homepage Pamper Zone</h3>
            <p>Welcome to Homepage Pamper Zone page</p>
          </div>
        </Col>
    ...
    <Wrapper>
      ...
      {addShowToast && (
        <SuccessToaster
          ...
          customMessage={`Please verify your entry once`}
        />
      )}
    </Wrapper>
  );
...
export default HomePagePamperZone;
